...
export default function ActivityLogs({ logs }: ActivityLogsProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="glass-card rounded-2xl overflow-hidden">
      <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-white/[0.06]">
        <div className="flex items-center gap-2.5">
          <Terminal className="w-4 h-4 text-text-muted" />
          <div><h3 className="text-[12px] sm:text-[13px] font-extrabold text-text-primary">Activity Logs</h3><p className="text-[10px] text-text-muted">{logs.length} entries</p></div>
        </div>
        <div className="flex items-center gap-1 px-2 py-0.5 rounded-md"><div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse" /><span className="text-[9px] font-bold text-success">LIVE</span></div>
      </div>
      <div ref={scrollRef} className="p-3 sm:p-4 max-h-[280px] sm:max-h-[320px] overflow-y-auto bg-surface-1 font-mono text-[10px] sm:text-[11px] leading-5">
        {logs.length === 0 ? (
          <div className="text-text-muted text-center py-8">Waiting for activity...▋</div>
        ) : (
          <>{logs.map((log, i) => {
            const style = typeStyles[log.type] || typeStyles.info;
            return (
              <motion.div key={i} initial={{ opacity: 0, x: -4 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.1 }} className="flex gap-2 px-1 py-0.5 rounded hover:bg-white/[0.02]">
