...
export default function DriveFilesViewer({ isConnected, refreshTrigger }: DriveFilesViewerProps) {
  ...
  const simulateUpload = (fileName: string) => {
    setUploadState({ active: true, progress: 0, name: fileName });
    const interval = setInterval(() => {
      setUploadState((prev) => {
        ...
        if (prev.progress >= 100) {
          clearInterval(interval);
          // Add a new mock file to the beginning of the list
          const newMockFile: DriveFile = {
            id: `mock-${Date.now()}`,
            name: fileName,
            mimeType: fileName.endsWith(".pdf") ? "application/pdf" : 
                      fileName.endsWith(".xlsx") || fileName.endsWith(".csv") ? "application/vnd.google-apps.spreadsheet" :
                      fileName.endsWith(".docx") ? "application/vnd.google-apps.document" : "application/vnd.google-apps.file",
            modifiedTime: new Date().toISOString(),
            size: `${Math.floor(Math.random() * 8000000) + 500000}`,
            owners: [{ displayName: "You", emailAddress: "jainjainam@412gmail.com" }],
            webViewLink: "https://drive.google.com"
          };
          setFiles((current) => [newMockFile, ...current]);
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-5 items-start">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              {/* Search Bar */}
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.08] text-[12px] w-full sm:w-48 focus-within:border-blue-500/40 transition-all">
      ...
      {/* iPadOS-style Details Inspection Drawer */}
      <AnimatePresence>
        {selectedFile && (
          <motion.div
            ...
            className="w-full lg:w-[280px] flex-shrink-0 select-none z-10"
          >
            <div className="glass-card rounded-2xl p-5 space-y-6 relative sticky top-24 border border-white/[0.08]">
              {/* Header Close button */}
              ...
              <div className="space-y-2 pt-2">
                {selectedFile.webViewLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-[12px] font-bold text-white bg-blue-500 hover:bg-blue-400 transition-colors shadow-lg shadow-blue-500/10 cursor-pointer select-none"
                  >
                    <ExternalLink className="w-3.5 h-3.5" />
                    Open in Drive
                  </a>
