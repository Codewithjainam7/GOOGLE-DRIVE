    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
        <div>
          ...
          <p className="text-[11px] text-text-muted mt-0.5 font-medium">OAuth 2.0 → Folder Lock handshake</p>
        </div>
        {currentStep > 0 && (
          <span className="text-[9px] font-bold uppercase tracking-wider text-blue-400 bg-blue-500/10 border border-blue-500/20 px-2.5 py-1 rounded-full w-fit">
          ...
          return (
            <div key={index} className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="flex flex-col items-center flex-shrink-0">
                {index > 0 && (
                  <div className={`w-[1.5px] h-3 transition-colors duration-500 ${
                    state !== "pending" ? "bg-emerald-500/40" : "bg-white/[0.05]"
                  }`} />
                )}
                ...
                <div className={`w-6 h-6 sm:w-7 sm:h-7 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-500 ${
                  state === "done" ? "bg-emerald-500/10 border border-emerald-500/25 shadow-[0_0_8px_rgba(16,185,129,0.08)]" :
                  state === "active" ? "bg-blue-500/10 border border-blue-500/30 shadow-[0_0_12px_rgba(59,130,246,0.12)] animate-pulse" :
                  "bg-white/[0.02] border border-white/[0.06]"
                }`}>
                  {state === "done" && <Check className="w-3 h-3 text-emerald-400" />}
                  {state === "active" && <Loader2 className="w-3 h-3 text-blue-400 animate-spin" />}
                  {state === "pending" && <Circle className="w-2 h-2 text-text-muted" />}
                </div>

                {!isLast && (
                  <div className={`w-[1.5px] h-3 transition-colors duration-500 ${
                    state === "done" ? "bg-emerald-500/40" : "bg-white/[0.05]"
                  }`} />
                )}
              </div>
              ...
              <span className={`text-[12px] sm:text-[13px] font-medium transition-colors duration-500 flex items-center gap-2 ${
                ...
                {isLast && <FolderLock className="w-3 h-3" />}
                {label}
              </span>
            </div>
          );
      ...
      <div className="mt-5 h-[3px] rounded-full bg-white/[0.03] overflow-hidden">
        <motion.div
          className="h-full rounded-full bg-gradient-to-r from-blue-500 via-indigo-500 to-emerald-500 shadow-[0_0_6px_rgba(59,130,246,0.4)]"
          initial={{ width: "0%" }}
